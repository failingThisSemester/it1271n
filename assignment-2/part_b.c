...
int main(int argc, char* argv[])
{
    ...
    if (n < 0)
        scream("read(stdin) failed");
    if (close(file) < 0)
        scream("close(write file) failed");

    // READ OPERATION

    file = open(argv[1], O_RDONLY);
    if (file < 0)
        scream("open(read) failed");
    talk("OUTPUT: ");
    ...
    if (n < 0)
        scream("read(file) failed");
    if (close(file) < 0)
        scream("close(read file) failed");
    return 0;
}
